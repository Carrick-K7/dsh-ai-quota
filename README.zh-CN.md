...
- **每个 provider 可单独显示 / 隐藏**：设置区标题栏的齿轮按钮点进去，就是嵌在同一个设置页里的第二层 —— 每个 provider 一个开关，并标注系统是否检测到。没检测到 Key / 登录态的 provider 仍列出，但开关不可用；检测到的一律默认开启，以后新配的 Key 会自动出现。开关对设置页列表**和**输入框额度行同时生效；偏好存在浏览器 `localStorage`，点一下即时生效，不走 host、不需要重启。`query_ai_quota` 工具不受影响，始终返回全部 provider。
