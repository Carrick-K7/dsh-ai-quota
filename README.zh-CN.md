...
- **DeepSeek / OpenCode Go**：配置指定的环境变量（默认 `DEEPSEEK_API_KEY` / `OPENCODE_GO_API_KEY`），回退 DSH 凭据 seam；OpenCode Go 额外回退 `~/.local/share/opencode/auth.json` 的 `opencode-go` 条目。
